// src/pages/dept-head/DeptRepository.jsx
...
const DeptRepository = () => {
  ...
  useEffect(() => {
    // Load data from local storage (simulating database repository)
    const storedRepo = JSON.parse(localStorage.getItem('fypRepository') || '[]');
    // Filter by current department
    const deptData = storedRepo.filter(item => item.department === department);
    setRepositoryData(deptData);
  }, [department]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Department Repository</h1>
          <p className="text-gray-500">Archived Final Year Projects - {department}</p>
        </div>
